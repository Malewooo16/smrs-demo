...
import SchoolEventCalendar from "@/app/main-components/SchoolCalender/MainCalender";
  ...
  return (
    <div className="py-4">
      <h2 className="text-2xl font-bold mb-2 px-10">School Event Calendar</h2>
        <div className="flex w-full justify-end py-2"><button className="btn btn-success"><Link className="flex h-full items-center" href={`/tcalendar/newevent`}>New Event</Link> </button>  </div>
        {events && events.length > 0 ? <SchoolEventCalendar events={events} /> : <p className="text-center">There are no events please add</p> }
    </div>
  )
}
