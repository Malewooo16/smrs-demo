...
export default async function Classes() {
  ...
  {classes && classes.length > 0 ? (
    classes.map((c, index) => (
     <Link href={`/students/${c.id}`} key={c.id}> <div key={index} className='card bg-base-200 w-96'>
     <p className='font-semibold text-lg px-6 py-4 rounded-tl-md rounded-tr-none'>{c.name}</p>
   </div></Link>
    ))
  ) : (
