...
import ValidateAdmissionData from "@/main-components/AdmissionData";
...
export default async function ValidateAdmission({
  searchParams,
}: {
  searchParams: {addyId: string; escuela: string};
}) {
  try {
    const admissionData = await getAdmissionById<IStudentAdmission | Error>(
      searchParams.addyId
    );
